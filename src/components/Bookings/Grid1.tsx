            ...
            selectedItems.map(([title, item]) => {
                ...
                return (
                    <View key={title} style={styles.selectedCard}>
                        ...
                        <Text style={styles.priceText}>₹{price} x {item.count} = ₹{price * item.count}</Text>
                    ...
                    {Object.values(cart).some((item) => item.count > 0) ? (
                        <>
                            <View style={styles.itemRow1}>
                                <Text style={[styles.itemText1, { flex: 2 }]}>Product</Text>
                            ...
                            {Object.entries(cart).map(([title, item]) =>
                                item.count > 0 ? (
                                    <View key={title} style={styles.itemRow2}>
                                        ...
                                        <Text style={[styles.itemText1, { flex: 1, textAlign: 'center' }]}>-</Text>
                                        <Text style={[styles.itemText1, { flex: 1, textAlign: 'center' }]}>{item.count}</Text>
...
const Card = ({ title, image, price, onQuantityChange, isService = false }) => {
        ...
        return (
            <View style={styles.card}>
                ...
                <TouchableOpacity
                    style={[
                        styles.addButton,
                        { backgroundColor: count > 0 ? '#aaa' : '#F93827' },
                    ]}
                    onPress={handleToggleService}
                >
                    <Text style={styles.addButtonText}>
                        {count > 0 ? 'Deselect' : 'Add'}
                    </Text>
                </TouchableOpacity>
            </View>
        );
 ...
 const style1= StyleSheet.create({
     container: {
         padding: 16, paddingBottom: 100, flex: 1,
         height: '80%',
},
     card: {
...
const styles = StyleSheet.create({
    ...
    cardText: { color: 'black', fontWeight: 'bold', textAlign: 'center', marginTop: 3 },
    priceText: { color: '#333', fontSize: 14, marginTop: 4 },
    counterRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 10 },
    counterButton: {
        backgroundColor: '#ddd',
    ...
    counterButtonText: { fontSize: 16 },
    counterValue: { fontSize: 16, fontWeight: 'bold' },
    totalText: { fontSize: 14, marginTop: 6, color: '#333' },
    totalListContainer: {
    ...
    itemText1: { fontSize: 16, color: '#333' },
    totalContainer: {
        backgroundColor: '#eee',
        padding: 16,
        marginTop: 10,
    ...
    totalAmount: { fontSize: 16, fontWeight: 'bold', color: '#F93827' },
    infoButton: {
        backgroundColor: 'red',
        padding: 12,
        marginTop: 12,
    ...
    tabButton: { paddingVertical: 8, paddingHorizontal: 16, borderRadius: 8, backgroundColor: '#ddd' },
    activeTabButton: { backgroundColor: '#F93827' },
    tabButtonText: { color: '#000', fontWeight: 'bold' },
